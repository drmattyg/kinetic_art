...
DELAY = 0.01
STEPS = 50
# d = digitalio.DigitalInOut(board.D6)
# p = digitalio.DigitalInOut(board.D7)
...
# coils = (
#     digitalio.DigitalInOut(board.D11),  # A1
#     digitalio.DigitalInOut(board.D10),  # A2
#     digitalio.DigitalInOut(board.D9),  # B1
#     digitalio.DigitalInOut(board.D8),  # B2
# )
coils = (
    pwmio.PWMOut(board.D2, frequency=1500),  # A1
    pwmio.PWMOut(board.D10, frequency=1500),  # A2
    pwmio.PWMOut(board.D9, frequency=1500),  # B1
    pwmio.PWMOut(board.D8, frequency=1500),  # B2
)
...
motor = stepper.StepperMotor(coils[0], coils[1], coils[2], coils[3], microsteps=16)
	# motor.onestep(style=stepper.SINGLE)
	# time.sleep(DELAY)
...
# for step in range(STEPS):
#     motor.onestep(style=stepper.MICROSTEP=4)
#     time.sleep(DELAY)
motor.release()
